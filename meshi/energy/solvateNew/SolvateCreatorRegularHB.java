...
public class SolvateCreatorRegularHB extends EnergyCreator  implements KeyWords {
    ...
    private EVenergyParametersList evParameters = null; // For the hydrogen bond definitions and distances
    ...
    public AbstractEnergy createEnergyTerm(Protein protein, DistanceMatrix distanceMatrix, 
		...
		if (parametersList== null) {
	    	...
	    	String[] strlist = new String[NEW_SOLVATE_PARAMETERS.length];
	    	String pathname = parametersDirectory(commands).concat("/");
	    	for (int cc=0 ; cc<NEW_SOLVATE_PARAMETERS.length ; cc++)
	        	strlist[cc] = pathname.concat(NEW_SOLVATE_PARAMETERS[cc]);
	    	parametersList = new SolvateParametersList(strlist);
	    	evParameters = new EVenergyParametersList(pathname+EXCLUDED_VOL_PARAMETERS_IMPROVED);
	 	}

		return new SolvateEnergy(protein.atoms(), 
					distanceMatrix, 
					(SolvateParametersList) parametersList,
					new HydrogenBondDahiyatList(distanceMatrix, protein.atoms(), evParameters),
